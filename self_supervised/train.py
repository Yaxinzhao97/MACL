...
#     parser.add_argument('--prefix', default='tmp0622', type=str, help='prefix of checkpoint filename')
...
def main(args):
    ...
    os.environ["CUDA_VISIBLE_DEVICES"] = str(args.gpu)  #默认使用0号gpu训练
    ...
    if args.model == 'memdpc':
        model = MemDPC_BD(is_bidirectional=True,  #双向循环神经网络
                          dropout=0.5,
                          network=args.net,
...
def train_one_epoch(train_data_loader, val_data_loader, model, criterion, optimizer, lr_scheduler, device, epoch, args, model_without_dp):
    batch_time = AverageMeter()  #取平均函数
    data_time = AverageMeter()
    losses = AverageMeter()
    accuracy = [[AverageMeter(), AverageMeter(), AverageMeter(), AverageMeter()],  # forward top1, top5
                ]  # backward top1, top5
